...
export default function MatchupCard({ matchup }: { matchup: Matchup }) {
  ...
  const getTeamLogo = useCallback((url: string | undefined) => {
    if (!url) return '/images/placeholder-team.png'
    return url.split('&')[0]
  }, [])
  ...
  return (
    <div
      ...
      onClick={() => {
        router.push(`/slate-manager/${matchup.slate_id}/${matchup.id}`)
      }}
    >
      <div className="flex flex-col md:flex-row items-center py-2 bg-card justify-center border-b border-border">
      ...
      {/* Teams */}
      <div className="flex flex-col px-4 pb-8 pt-2 sm:flex-row items-center justify-between gap-4 sm:gap-0">
        {/* Away Team */}
        <div className="flex items-center w-full sm:w-[40%] text-lg text-center sm:text-left">
          ...
          <div className="flex flex-col items-left text-left text-2xl ml-4">
            <div>{matchup.away_team_city}</div>
            <h3 className="uppercase text-4xl font-bold hidden md:block">{matchup.away_team_name}</h3>
          </div>
        </div>
        ...
        <div className="flex flex-row md:flex-row-reverse items-center w-full sm:w-[40%] text-lg text-center sm:text-right">
          ...
          <div className="flex flex-col items-right text-2xl ml-4 md:mr-4">
      ...
      {/* Vegas Totals */}
      <div className="flex bg-card text-sm justify-between rounded-t text-muted px-6 py-2 border-t border-border">
