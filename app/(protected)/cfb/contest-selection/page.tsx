...
const normalizeContest = (c: RawContest, idx: number): ContestView => {
  ...
  const infer3Max =
    lowerName.includes('3-max') || lowerName.includes('3 entry max') || lowerName.includes('3-entry max')
  const infer20Max =
    lowerName.includes('20-max') || lowerName.includes('20 entry max') || lowerName.includes('20-entry max')
  const infer150Max =
  ...
  for (const c of filtered) {
    ...
    if (isCash) {
      ...
      if (passesField && passesFirstPct) {
        ...
        if (passesFirstPct && c.firstPct != null) reasons.push(`1st% ${(c.firstPct * 100).toFixed(1)}% ≤ 20%`)
      ...
      <div className="flex items-center justify-between mb-4 px-2">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted text-sm">
            The following tool will help you pick contests for your slate by building an aggressive profile and picking
            contests that fit that profile.
          </p>
        </div>
        ...
        <CardContent className="space-y-4">
          <div className=" flex items-center gap-8">
            ...
            <div>
              ...
              <Select value={allocation} onValueChange={v => setAllocation(v as 'aggressive' | 'normal')}>
            ...
            <div>
              ...
              <Select
                value={bucketFilter}
                onValueChange={v => setBucketFilter(v as 'ALL' | 'Cash' | 'GPP-SE' | 'GPP-3Max' | 'GPP-20Max')}
              >
            ...
            <div className="flex items-center gap-3 pt-6">
              <Switch checked={showOnlyRecommended} onCheckedChange={setShowOnlyRecommended} id="rec-only" />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="rounded">
          ...
          <CardContent className="space-y-1 text-sm py-4">
            ...
            <div>
              Cash budget ({Math.round(result.cashPct * 100)}%): <b>${result.cashBudget.toFixed(2)}</b>
            </div>
            <div>
