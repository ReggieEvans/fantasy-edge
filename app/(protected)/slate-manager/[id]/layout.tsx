...
import { Binoculars, Brain, ClipboardCheck, Cog, Hammer, MousePointerClick, Users } from 'lucide-react'
...
export default function SlateLayout({ children }: { children: React.ReactNode }) {
  ...
  const isLinkActive = (linkHref: string) => {
    const basePath = `/slate-manager/${id}`
    if (linkHref === '') {
      return (
        pathname === basePath ||
  ...
  const renderNavLink = (href: string, label: string, icon: ReactNode) => (
    <Link
      key={label}
      href={`/slate-manager/${id}/${href}`}
      className={`flex justify-center items-center gap-3 w-full px-6 py-4 border-l-[4px] transition-all duration-300 ${
        isLinkActive(href)
