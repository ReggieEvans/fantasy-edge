      ...
      <div className="space-y-4 px-6">
        {isLoading ? (
          <div className="flex flex-wrap gap-4">
            {[...Array(6)].map((_, i) => (
              <Skeleton key={i} className="w-[500px] h-[600px] bg-card rounded animate-pulse" />
            ))}
          </div>
        ) : rosters?.length === 0 ? (
          <div className="flex flex-col items-center justify-center max-w-[350px] mx-auto pt-24 space-y-2">
            <h2 className="text-muted text-center text-lg font-bold opacity-70">No Rosters found</h2>
            <p className="text-muted text-center text-sm opacity-50">
              You currently have no rosters for this slate. You can create a roster by selecting the build button in the
              side navigation.
            </p>
          </div>
        ) : (
