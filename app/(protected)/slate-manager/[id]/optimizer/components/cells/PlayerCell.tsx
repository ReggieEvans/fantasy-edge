...
export const ExcludeCell = memo(({ player, onToggleExclude }: Pick<PlayerCellProps, 'player' | 'onToggleExclude'>) => {
  ...
  return (
    <div className="flex justify-center w-8">
      ...
      >
        {isExcluded ? <Square className="w-4 h-4 text-muted" /> : <CircleX className="w-4 h-4 text-destructive" />}
      </button>
    </div>
  )
})
...
export const LockCell = memo(({ player, onToggleLock }: Pick<PlayerCellProps, 'player' | 'onToggleLock'>) => {
